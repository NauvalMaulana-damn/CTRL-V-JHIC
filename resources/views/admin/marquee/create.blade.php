...
<x-admin-layout>
    ...
    <form action="{{ route('admin.marquee.store') }}" method="POST" enctype="multipart/form-data" class="space-y-4 bg-white p-6 rounded shadow">
        @csrf

        <div>
            <label class="block mb-1 font-medium">Nama Logo</label>
            <input type="text" name="nama" value="{{ old('nama') }}" class="w-full border rounded p-2" required maxlength="100" placeholder="Contoh: Google, Microsoft, dll.">
        </div>
        ...
        <div>
            ...
            <input type="number" name="urutan" value="{{ old('urutan', 0) }}" class="w-full border rounded p-2" min="0" placeholder="0">
            <p class="text-sm text-gray-500 mt-1">Angka lebih kecil akan tampil lebih dulu</p>
        </div>

        <div class="flex justify-end space-x-2">
            <a href="{{ route('admin.marquee.index') }}" class="bg-gray-500 text-white px-4 py-2 rounded hover:bg-gray-600">Kembali</a>
